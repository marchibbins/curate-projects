...
class Agnosia_Bootstrap_Carousel {
	...
	private function get_meta( $post ) {
		...
		$output = '';
		if ( 'false' !== 'text' ) {
			$output .= ( $wpautop != 'false' ) ? wpautop( get_field( 'materials' , $post['ID'] ) ) : get_field( 'materials' , $post['ID'] );
			$output .= ( $wpautop != 'false' ) ? wpautop( get_field( 'dimensions' , $post['ID'] ) ) : get_field( 'dimensions' , $post['ID'] );
		}
		$output = apply_filters( 'agnosia_bootstrap_carousel_excerpt', $output, $this->attributes );
		return $output;
	}
